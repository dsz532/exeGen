...
# axs.set_ylim(0.02, max(Single_Choice) + 0.1)
# axs.set_xticks([width + width / 2, 8 * width], ['HR', 'Recall'])
...
# axs.set_ylim(0.02, max(Multiple_Choice) + 0.1)
# axs.set_xticks([width + width / 2, 8 * width], ['HR', 'Recall'])
...
axs.set_ylim(0.02, max(Judgment) + 0.1)
axs.set_xticks([width + width / 2, 8 * width], ['HR', 'Recall'])
...
# 调整布局
plt.tight_layout(rect=[0, 0, 1, 0.8])  # 为图例留出空间

plt.savefig('txtfile/fig3.png', dpi=600, bbox_inches='tight')
